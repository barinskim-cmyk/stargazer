...
**Date:** March 2026
**Total modules:** 32 (5 systems + 11 engines + 16 screens)
**index.html:** 2,765 lines (HTML/CSS + game core logic)
**Total extracted:** ~8,954 lines across 32 modules
...
```
www/
  index.html              — HTML, CSS, canvas init, game loop, core state
  systems/                — Infrastructure (load first)
    ...
    offline-queue.js      — Offline save queue, sync, network listeners
  engines/                — Game logic & rendering (load second)
    ...
    tutorial-overlay.js   — Tutorial tooltip + pulse overlay
    hints-renderer.js     — Hint edge rendering (all themes)
    win-system.js         — Win particles, endGame, finishEndGame
    canvas-input.js       — Pointer/click handlers on canvas (post-inline)
  screens/                — UI screens (load third, after Supabase init)
    ...
    victory-duel.js       — Split-screen constellation duel
    rank-up.js            — Rank-up ceremony + progress widget
    save-animation.js     — Save-to-sky fly animation
    menu-navigation.js    — goToMenu, fadeTo, refreshHomeConstellations
    victory-handlers.js   — Victory buttons, save logic (post-inline)
```
...
<script src="engines/audio.js"></script>
<script src="engines/tutorial-overlay.js"></script>
<script src="engines/hints-renderer.js"></script>
<script src="engines/win-system.js"></script>
...
<script src="screens/victory-duel.js"></script>
<script src="screens/rank-up.js"></script>
<script src="screens/save-animation.js"></script>
<script src="screens/menu-navigation.js"></script>
<script src="systems/offline-queue.js"></script>

<!-- 7. Main game script (inline) — canvas, draw(), startGame(), core state -->
<script> ... </script>

<!-- 8. Post-inline modules (need vars from inline script) -->
<script src="engines/canvas-input.js"></script>
<script src="screens/victory-handlers.js"></script>
```
...
### engines/audio.js (85 lines)

Sound effects via Web Audio API (oscillator synthesis, no audio files).

| Export | Description |
|--------|-------------|
| `playSound(type)` | Play sound by type: `move`, `score`, `victory`, `twinkle`, `click` |

**Dependencies:** `window._appSettings.sound`
**Load-time side effects:** none (AudioContext created on first call)

---

### systems/offline-queue.js (128 lines)

Offline save queue with idempotency keys for reliable Supabase sync.

| Export | Description |
|--------|-------------|
| `_uuid4()` | Generate UUID v4 |
| `_queueSkySave(payload)` | Queue a constellation save for offline sync |
| `_updateQueueBadge()` | Show/hide pending saves badge |
| `_syncOfflineQueue()` | Sync all queued saves to Supabase |
| `_onNetworkChange(online)` | Handle network status changes |

**Dependencies:** `sb`, `t()`
**Load-time side effects:** registers `online`/`offline` event listeners

---

### engines/tutorial-overlay.js (186 lines)

In-game tutorial overlay: pulsing star highlights, tooltip bubbles, step progression.

| Export | Description |
|--------|-------------|
| `showTutorialTooltip(text, rect)` | Show tooltip bubble near target rect |
| `startDismissTutorial()` | Begin tutorial dismissal animation |
| `checkTutorialMoments(a, b)` | Check if a move triggers tutorial step |
| `ensureTutorialLoop()` | Start tutorial animation RAF loop |
| `drawTutorialOverlay()` | Render tutorial overlay on main canvas |

**Dependencies:** `canvas`, `ctx`, `W`, `H`, `_rrect()`, `_tutorialStep`, `_tutorialPulseIdx`, `RADIUS`
**Load-time side effects:** none

---

### engines/hints-renderer.js (98 lines)

Hint edge rendering per theme (chalk dots, stargazer shimmer, dashed lines).

| Export | Description |
|--------|-------------|
| `drawHints()` | Draw hint edges on main canvas |
| `ensureHintsLoop()` | Start hints animation RAF loop |

**Dependencies:** `ctx`, `W`, `H`, `points`, `RADIUS`, `activeTheme`, `THEMES`, chalk drawing functions
**Load-time side effects:** none

---

### screens/victory-duel.js (220 lines)

Split-screen constellation duel view showing player vs AI constellations across all 5 themes.

| Export | Description |
|--------|-------------|
| `drawVictoryDuel(cvs, data)` | Render duel on a canvas |

**Dependencies:** `LC`, `RADIUS`, `THEMES`, `activeTheme`, `drawStarPointTo()`, chalk drawing functions, victory-renderer functions
**Load-time side effects:** none

---

### engines/win-system.js (226 lines)

End-of-game orchestration: win particles, endGame flow, finishEndGame, probability bar animation.

| Export | Description |
|--------|-------------|
| `startWinEffect()` | Start win particle explosion |
| `endGame()` | Begin end-of-game sequence (score, save, progression) |
| `finishEndGame()` | Finalize game (show victory screen) |
| `startProbBarAnim()` | Animate probability bar to current value |

**Dependencies:** `canvas`, `ctx`, `W`, `H`, `points`, `lines`, `scores`, `polygons`, `turn`, `COUNT`, `isAI`, `playerNames`, `stargazerLevel`, `resolveGame()`, `computeGamePoints()`, `progressionUpdate()`, `loadProgress()`, `computeRank()`, `_persistSaved()`, `_loadSaved()`, `saveToSkyAnimation()`, `startRankUpCeremony()`, `renderProgressWidget()`, `drawVictoryDuel()`, `playSound()`, `haptic()`
**Load-time side effects:** none

---

### screens/rank-up.js (114 lines)

Rank-up ceremony animation (emblem scale-in with particles) and progress widget rendering.

| Export | Description |
|--------|-------------|
| `startRankUpCeremony(rank)` | Animate rank-up ceremony |
| `renderProgressWidget(data)` | Render post-game progress bar |

**Dependencies:** `ctx`, `W`, `H`, `LC`, `drawRankEmblem()`, `getRankName()`, `RANK_THRESHOLDS`, `t()`
**Load-time side effects:** none

---

### screens/save-animation.js (232 lines)

Full-screen save-to-sky animation: star launch, blink effect, sky background, text overlay.

| Export | Description |
|--------|-------------|
| `_drawSkyStar(ctx, s, t)` | Draw a single sky star with twinkle |
| `saveToSkyAnimation(name, data)` | Run full save animation sequence |

**Dependencies:** `LC`, `RADIUS`, `drawStarPointTo()`, `_skyEnsureBg()`, `_skyGetUserId()`, `_SKY_WORLD`, `_bgStars`, `THEMES`, `activeTheme`, `t()`, `playSound()`
**Load-time side effects:** none

---

### engines/canvas-input.js (118 lines)

Canvas pointer and click event handlers with tutorial intercepts.

| Export | Description |
|--------|-------------|
| *(no exports)* | Registers `pointermove`, `pointerleave`, `click` on `canvas` |

**Dependencies:** `canvas`, `ctx`, `W`, `H`, `points`, `RADIUS`, `currentScreen`, `startDismissTutorial()`, `applyMove()`, `showTutorialTooltip()`, `checkTutorialMoments()`
**Load-time side effects:** registers 3 event listeners on `canvas`
**Load order:** must load AFTER inline script (needs `canvas` var)

---

### screens/menu-navigation.js (116 lines)

Master screen transitions: menu reset, fade animation, home constellation refresh.

| Export | Description |
|--------|-------------|
| `goToMenu()` | Full reset: cancel RAFs, clear state, return to home |
| `fadeTo(screen)` | Animated screen transition |
| `refreshHomeConstellations()` | Reload constellation display on home screen |

**Dependencies:** `_loadSaved()`, `drawMiniConstellation()`, `showHomeScreen()`, `_startHomeBgAnim()`, `_stopHomeBgAnim()`, `currentScreen`, various RAF handles
**Load-time side effects:** none

---

### screens/victory-handlers.js (211 lines)

Victory screen button handlers, save logic, hints toggles, tutorial auto-save.

| Export | Description |
|--------|-------------|
| `applyGuestMode()` | Apply guest profile defaults |
| `_doTutorialAutoSave()` | Auto-save tutorial constellation |

**Dependencies:** `canvas`, `points`, `lines`, `scores`, `polygons`, `currentScreen`, `isAI`, `COUNT`, `playerNames`, `goToMenu()`, `fadeTo()`, `startGame()`, `endGame()`, `_loadSaved()`, `_persistSaved()`, `_queueSkySave()`, `_uuid4()`, `saveToSkyAnimation()`, `refreshHomeConstellations()`, `showToast()`, `playSound()`, `haptic()`, `t()`
**Load-time side effects:** registers click handlers on victory/hint/save buttons
**Load order:** must load AFTER inline script (needs DOM + game state vars)

---

## Remaining in index.html (2,765 lines)

Content that stays in index.html — core game state and tightly coupled logic:

| Section | ~Lines | Why not extracted |
|---------|--------|-------------------|
| HTML structure | 1,400 | DOM elements for all screens |
| CSS styles | 530 | All styling including media queries |
| Early init (haptic, toast, settings) | 40 | Must run before everything |
| Supabase init + shared globals | 20 | `sb`, `_proceedFromAuth` |
...
| Game state vars | 60 | `points`, `lines`, `scores`, `turn`, etc. |
| Game loop (draw, applyMove, startGame) | 400 | Core draw loop + game state |
...
### Deep decomposition (March 2026)

Extracted 12 additional modules from index.html across 5 rounds (4,236 → 2,765 lines, −35%):

- **Round 1:** audio.js (sound effects), offline-queue.js (save queue)
- **Round 2:** tutorial-overlay.js, hints-renderer.js, victory-duel.js
- **Round 3:** win-system.js (endGame flow), rank-up.js
- **Round 4:** save-animation.js, canvas-input.js (pointer handlers)
- **Round 5:** menu-navigation.js (goToMenu, fadeTo), victory-handlers.js (button handlers)

Key pattern discovered: modules that register event listeners needing `var` globals from the inline script must load AFTER the inline `</script>` tag (canvas-input.js, victory-handlers.js).
...
**Exception:** `canvas-input.js` and `victory-handlers.js` register event listeners at load time (IIFEs run immediately). They need `canvas`, `points`, and other `var` globals declared in the inline script. These two modules load AFTER the inline `</script>` tag (section 8 in load order).
